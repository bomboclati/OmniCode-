class DocsPanel {
    ...
    async loadDocs() {
        try {
            ...
            console.warn('Failed to load docs, using demo content:', e);
            this.content = `# OmniCode API Reference

## Overview
OmniCode provides a WebSocket-based real-time API and a RESTful HTTP API.
...
\`\`\`json
{
  "message": "build a REST API with Rust",
  "session": "abc-123"
}
\`\`\`
...
### GET /api/files
List project files.
Returns a JSON array of file paths and metadata.

### GET /api/search?q=<query>
Semantic code search across the project cortex.
...
- \`file_change\` — Sync file edits
- \`cursor_update\` — Broadcast cursor position
- \`system_event\` — Agent status updates

## Agent Tools
The OmniCode agent supports these tool categories:
- **Code** — read, write, edit, search files
- **Shell** — execute commands in sandbox
- **Git** — commit, branch, diff, merge
- **Web** — fetch URLs, search docs
- **AI** — ask LLM, generate embeddings`;
        }
        this.render();
    }
